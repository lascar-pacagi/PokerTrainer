...
@dataclass
class ModelConfig:
    # Flat-sequence encoding (v0.2 — see docs/STATE_ENCODING.md).
    # x now includes the full flat action history + valid-mask.
    x_dim: int = 745         # must match engine X_DIM
    a_dim: int = 17          # must match engine A_DIM
    mlp_hidden: int = 512
    mlp_layers: int = 5
    # arch:
