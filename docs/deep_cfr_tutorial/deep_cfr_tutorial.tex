...
def regret_matching(R, legal):              # R: regret per action
    ...
    # all regrets non-positive: play the least-bad action(s), splitting
    # probability equally across exact ties (so a zero-init all-equal R
    # gives uniform-over-legal: the t=1 bootstrap discussed below).
    best = max(R[a] for a in legal)
    ties = [a for a in legal if R[a] == best]
    return {a: (1.0 / len(ties) if a in ties else 0.0) for a in legal}
\end{lstlisting}
...
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=2mm]}]
  ...
  \node[align=center, anchor=south] at (6.0,0.30)
    {\textbf{Deep CFR}: no abstraction\\ \textbf{Pluribus}: 6-max\\ superhuman};
...
Deep CFR       & 2019 & networks replace regret tables              & no  & advantage+policy \\
