...
Suppose at some decision point you have played many times, and looking back:
playing \texttt{fold} would have earned you $-2$ chips relative to what you
actually did on average, \texttt{call} $+3$, and \texttt{raise} $+1$.
Your (cumulative) regrets are $R = [-2, +3, +1]$. \textbf{Regret matching}
(Hart \& Mas-Colell, 2000) converts these into a strategy:
...
matching, repeat. Two practical refinements matter for us:
...
\item \textbf{CFR$^{+}$}: clip cumulative regrets at zero. (Used by the
      commercial GTO solvers; Deep CFR uses Linear CFR instead.)
\end{itemize}
