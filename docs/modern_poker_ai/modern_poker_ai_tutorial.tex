...
\begin{trap}
\textbf{Where the soundness leak is.} Step 2 \emph{assumes} opponents played
the blueprint so far. In two-player zero-sum, this ``unsafe'' re-solving can
in principle be punished by an opponent who deviates earlier on purpose to
poison your beliefs; Libratus closed exactly this hole with \emph{safe}
re-solving gadgets that guarantee the re-solved strategy never does worse
than the blueprint against any hand. Pluribus --- playing six-max, where
equilibrium guarantees were abandoned anyway (Sec.\ 2.1) --- accepts the
heuristic. One more instance of the multiplayer trade: theory exchanged for
scale, validated empirically.
\end{trap}
