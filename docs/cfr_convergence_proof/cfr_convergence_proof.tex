...
information sets, and $A$ the maximum number of actions at any of them --- with
$|\mathcal{I}|$ and $A$ taken as the \emph{larger} of the two players' values,
so this symmetric form is a valid worst case (the exact, possibly asymmetric,
bound is assembled in Sec.~6). In particular $\varepsilon \to 0$: average CFR
self-play converges to GTO.
\end{thmbox}
...
$\varepsilon$ --- the \emph{larger} of the two players' best-response gains ---
measures the profile's distance from equilibrium. \textbf{Convention warning:}
``exploitability'' is also used (including by this repo's
\texttt{exploitability\_bb}) for the related but distinct quantity that
\emph{averages} the two best-response gains, $\tfrac12(\delta_1+\delta_2)$, in
bb/hand. The two differ by at most a factor of two and the convergence
statement is unaffected --- but do not equate the raw numbers across
conventions.
\end{defbox}

\begin{defbox}
\textbf{Average strategy.} $\bar\sigma_i = \frac1T\sum_{t=1}^{T}\sigma_i^t$ ---
the mixture that plays a uniformly random past round's strategy. (In an
\emph{extensive-form} game this averaging must be \emph{reach-weighted}, not
arithmetic, or the linearity step below breaks --- see the \textbf{Watch out}
box right after Theorem~B; in this section's single-decision setting the two
coincide.) The single property we need is linearity of expected payoff in each
argument:
\[
...
\emph{Proof sketch (the full induction is in Zinkevich et al.\ 2007).} The
global regret compares
...
Formally, one shows for every infoset $I$ the \emph{raw} recurrence
\[
...
steers toward (adding non-negative terms only loosens the bound). Now take the
positive part of both sides: since the child sum is already non-negative and
$\pos{x+y} \le x^{+}+y$ whenever $y \ge 0$, the raw recurrence becomes the
\emph{clipped} form that matches the theorem statement,
\[
\pos{R_T^{\mathrm{full}}(I)} \;\le\; \pos{R_T^{\mathrm{imm}}(I)}
\;+\; \sum_{I' \in \mathrm{succ}(I)} \pos{R_T^{\mathrm{full}}(I')}.
\]
Unrolling from the leaves (where $R^{\mathrm{full}} = R^{\mathrm{imm}}$, hence
$\pos{R^{\mathrm{full}}} = \pos{R^{\mathrm{imm}}}$) up to the root collapses the
nested sums into one flat $\sum_{I}\pos{R_T^{\mathrm{imm}}(I)}$. At the root the
at-and-below regret \emph{is} the whole-game regret, so
$R_T^{\mathrm{game}} \le \pos{R_T^{\mathrm{game}}}
\le \sum_{I}\pos{R_T^{\mathrm{imm}}(I)}$. $\square$
...
\textbf{Asymmetric games.} The $\varepsilon_i$ above is really
$\varepsilon_i = |\mathcal{I}_i|\,\Delta\sqrt{A_i}/\sqrt{T}$, built from player
$i$'s \emph{own} infoset count and action count, so the exact bound is
\[
\varepsilon \;\le\; \frac{\Delta\bigl(|\mathcal{I}_1|\sqrt{A_1}
   + |\mathcal{I}_2|\sqrt{A_2}\bigr)}{\sqrt{T}}.
\]
Writing $|\mathcal{I}|,A$ for the larger of the two players' values collapses
this to the symmetric $2\,\Delta\,|\mathcal{I}|\sqrt{A}/\sqrt{T}$ quoted in
Section~1. (Heads-up poker is nearly symmetric, so the gap is small.)
...
\textbf{Faster variants, same skeleton.} CFR$^{+}$ (regret-matching$^{+}$:
clip the \emph{cumulative} regrets to $\ge 0$, with alternating updates and
linear averaging) and Linear CFR (weight round $t$'s contribution by $t$)
reuse the same machinery --- the potential argument tolerates the extra
clipping (Lemma~2 is exactly the inequality needed) and the round-weighting
(carry $\sum_t t\,r_t$; the orthogonality identity is unchanged). Their
advantage is \emph{primarily empirical}: both converge several times faster in
practice. Matching worst-case \emph{bounds} are subtler --- CFR$^{+}$ for years
carried only the same $O(1/\sqrt{T})$ guarantee as plain CFR despite its speed,
and Linear CFR's improvement comes with its own discount-dependent analysis
(Tammelin 2014; Brown \& Sandholm 2019). This project's trainer uses Linear CFR.
